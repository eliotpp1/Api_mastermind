...
class Model
{
    ...
    public function create(
        ...
        // Vérifier que toutes les données sont des valeurs scalaires
        foreach ($datas as $key => $value) {
            if (is_array($value) || is_object($value)) {
                throw new InvalidArgumentException("La valeur pour $key doit être une chaîne, un nombre ou une valeur scalaire.");
            }
        }
        ...
        // Préparer les attributs pour l'insertion
        foreach (array_keys($datas) as $k) {
    ...
    public function delete(
        ...
        $sql = "DELETE FROM `{$this->tableName}` WHERE game_id = :id";
        $sth = $this->query($sql, [':id' => $id]);
        return $sth->rowCount() > 0;
    }
